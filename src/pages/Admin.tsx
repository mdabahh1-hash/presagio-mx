...
export function Admin() {
  ...
  const resolve = async (marketId: string, resolution: 'YES' | 'NO') => {
    ...
    try {
      ...
      setMessage(`✓ Mercado resuelto como ${result.resolution}. ${result.positions_settled} posiciones liquidadas.`)
      const updated = await adminApi.listAllMarkets()
      setMarkets(updated)
    } catch (e: unknown) {
              ...
              <div key={m.id} style={{
                ...
                <div style={{ flex: 1, minWidth: 200 }}>
                  ...
                  <div style={{ display: 'flex', gap: 12, fontSize: '0.75rem', color: 'var(--text-tertiary)' }}>
                    ...
                    <span>YES: {m.yes_price.toFixed(1)}%</span>
                  </div>
                </div>
              ...
              {resolved.map(m => {
                ...
                return (
                  <div key={m.id} style={{
                    ...
                    <span style={{ fontSize: '0.8rem', fontWeight: 700, color: st.color }}>{st.label}</span>
                  </div>
                )
              })}
            </>
