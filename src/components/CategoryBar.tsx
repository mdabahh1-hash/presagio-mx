...
// Feeds (no son categorías del API): Tendencia y Nuevo, con icono, separados
// de las categorías por una línea vertical (estilo Polymarket).
export const FEEDS = ['Tendencia', 'Nuevo'] as const
...
// /mercados?cat=) y el activo se lee de la URL (Perfil, Mercados).
...
export function CategoryBar({ active, onChange, sticky = true, children, style }: Props) {
  ...
  const items: TabItem<CategoryTab>[] = TABS.map(tab => ({
    key: tab,
    label: tab === 'Tendencia' ? t('home.tabTrending') : tab === 'Nuevo' ? t('home.tabNew') : tab,
    icon: tab === 'Tendencia' ? 'trending' : tab === 'Nuevo' ? 'sparkle' : undefined,
