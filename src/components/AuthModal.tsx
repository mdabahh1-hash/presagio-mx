...
interface AuthModalProps {
  ...
  hidePasskey?: boolean
  // Sin overlay ni panel propios: el contenido va dentro de otra superficie (la hoja
  // de compra). Cerrar o terminar el acceso devuelve a esa superficie vía onClose.
  embedded?: boolean
  // `next` del OAuth cuando no es la ruta actual (volver a la compra en el detalle)
  oauthNextRoute?: string
}

export function AuthModal({ onClose, initialMode = 'login', hidePasskey = false, embedded = false, oauthNextRoute }: AuthModalProps) {
  ...
  useEffect(() => {
    // Embebido, el Escape y el bloqueo de scroll son de la hoja que lo contiene
    if (embedded) return
  ...
  if (embedded) {
    return (
      <div>
        <button type="button" onClick={onClose} className="btn btn-ghost btn-sm" style={{ margin: '0 0 12px -8px' }}>
