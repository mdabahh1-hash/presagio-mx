/**
 * Vista previa de una jornada: cuántos mercados entran, cuándo cierra el
 * último, el nombre (automático y editable si llega `onNameChange`) y las
 * primeras preguntas con miniatura. Con 0 mercados muestra el estado vacío.
 *
 * La usan CreateLeaguePage (editable) y LeagueLobby (solo lectura).
 */
...
export type PreviewMarket = Pick<Market, 'id' | 'question' | 'endsAt' | 'category' | 'subcategory' | 'imageUrl'> &
  Partial<Pick<Market, 'outcomes'>>

interface Props {
  markets: PreviewMarket[]
  /** Nombre efectivo de la jornada; se muestra solo si viene. */
  name?: string
  nameEdited?: boolean
  /** Si viene, el nombre se puede editar inline. */
  onNameChange?: (name: string, edited: boolean) => void
  /** Etiqueta de la competencia para el estado vacío. */
  subcategoryLabel: string
  title?: string
  /** Contenido extra en la cabecera (p. ej. countdown al primer cierre). */
  extra?: ReactNode
}

export function CyclePreview({ markets, name, nameEdited = false, onNameChange, subcategoryLabel, title, extra }: Props) {
  const { t } = useTranslation()
  const [expanded, setExpanded] = useState(false)
  const [editing, setEditing] = useState(false)
  const [draft, setDraft] = useState(name ?? '')

  const sorted = [...markets].sort((a, b) => Date.parse(a.endsAt) - Date.parse(b.endsAt))
  ...
  return (
    <section className="card lg-preview" aria-live="polite">
      <div className="lg-preview__head">
        <h2 className="lg-preview__title">{title ?? t('leagues.create.previewTitle')}</h2>
        {lastClose && (
      ...
      {extra && <div className="lg-preview__extra">{extra}</div>}
